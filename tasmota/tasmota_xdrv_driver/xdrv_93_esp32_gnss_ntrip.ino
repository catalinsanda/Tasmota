...
{
public:
  static constexpr uint32_t RECONNECT_DELAY = 30000;
  static constexpr size_t MAX_QUEUE_SIZE = 24576; // 24KB queue size

    NTRIPClient(uint8_t index)
      : connected(false), enabled(false), connecting(false), retryCount(0),
        nextRetryTime(0), lastConnectAttempt(0), shouldReconnect(false), port(0),
        sendInProgress(false), droppedPackets(0)
  {
    client = new AsyncClient();
    setupClientCallbacks();
    sendQueue.reserve(MAX_QUEUE_SIZE);
  ...
  void sendData(const uint8_t *data, size_t length)
  {
    ...
    {
      AddLog(LOG_LEVEL_ERROR, PSTR("NTRIPC[%d]: Packet too large: %d bytes"), index, length);
      return;
    }

    // If queue is getting too full, drop oldest data
    if (sendQueue.size() + length >= MAX_QUEUE_SIZE)
    {
      size_t dropSize = (sendQueue.size() + length) - MAX_QUEUE_SIZE;
      sendQueue.erase(sendQueue.begin(), sendQueue.begin() + dropSize);
      droppedPackets++;
      AddLog(LOG_LEVEL_ERROR, PSTR("NTRIPC[%d]: Dropped %d bytes from queue (total dropped packets: %d)"),
             index, dropSize, droppedPackets);
    }

    sendQueue.insert(sendQueue.end(), data, data + length);
  ...
  {
    if (!enabled || !connected || !client || !client->connected() || sendInProgress || sendQueue.empty())
    {
      return;
    }
    ...
    size_t availableSpace = client->space();
    size_t toSend = std::min(availableSpace, sendQueue.size());
    // toSend = std::min(toSend, (size_t)1460); // Limit to TCP MSS to avoid fragmentation

    if (toSend > 0)
    {
      size_t written = client->write(reinterpret_cast<const char *>(sendQueue.data()), toSend);

      if (written > 0)
      {
        sendQueue.erase(sendQueue.begin(), sendQueue.begin() + written);

        AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("NTRIPC[%d]: Sent %d bytes, %d bytes remaining in queue"),
               index, written, sendQueue.size());
      }
      else
      {
        AddLog(LOG_LEVEL_ERROR, PSTR("NTRIPC[%d]: Failed to write data"), index);
      }
    }
  ...
  size_t getQueuedBytes() const { return sendQueue.size(); }
  uint32_t getDroppedPackets() const { return droppedPackets; }
  void resetDroppedPackets() { droppedPackets = 0; }
...
private:
  ...
  void setupClientCallbacks()
  {
    ...
    client->onAck([this](void *obj, AsyncClient *c, size_t len, uint32_t time)
                  { handleAck(c, len, time); });

    client->onTimeout([this](void *obj, AsyncClient *c, uint32_t time)
                      { handleTimeout(c, time); });

    client->setRxTimeout(30);
  ...
  void initiateConnect()
  {
    ...
    AddLog(LOG_LEVEL_INFO, PSTR("NTRIPC[%d]: Connecting to %s:%d (attempt %d)"), index, host, port, retryCount);
  ...
  void handleTimeout(AsyncClient *c, uint32_t time)
  {
    if (c != client)
      return;

    AddLog(LOG_LEVEL_ERROR, PSTR("NTRIPC[%d]: Connection timeout after %dms, will disconnect"), index, time);
    connected = false;
    connecting = false;
    shouldReconnect = true;
    ...
    if (enabled)
    {
      ...
      AddLog(LOG_LEVEL_INFO, PSTR("NTRIPC[%d]: Will retry in %d seconds"), index, RECONNECT_DELAY / 1000);
    }
  ...
  void handleAck(AsyncClient *c, size_t len, uint32_t time)
  {
    if (c != client)
      return;
    ...
    if (time > 8000)
    {
      AddLog(LOG_LEVEL_ERROR, PSTR("NTRIPC[%d]: Slow ACK detected: %dms"), index, time);
    }

    // Try to send more data from queue
    processSendQueue();
  }
  ...
  void handleConnect(AsyncClient *c)
  {
    ...
    AddLog(LOG_LEVEL_INFO, PSTR("NTRIPC[%d]: Connected to %s:%d"), index, host, port);
  ...
  void handleDisconnect(AsyncClient *c)
  {
    ...
    AddLog(LOG_LEVEL_INFO, PSTR("NTRIPC[%d]: Disconnected from %s:%d, %d bytes lost in queue"),
           index, host, port, sendQueue.size());
  ...
  void handleError(AsyncClient *c, int8_t error)
  {
    ...
    AddLog(LOG_LEVEL_ERROR, PSTR("NTRIPC[%d]: Connection error %d"), index, error);
  ...
  void handleData(AsyncClient *c, uint8_t *data, size_t len)
  {
    ...
    {
      AddLog(LOG_LEVEL_DEBUG, PSTR("NTRIPC[%d]: Server accepted connection"), index);
    }
    else if (strstr(response, "401"))
    {
      AddLog(LOG_LEVEL_ERROR, PSTR("NTRIPC[%d]: Authentication failed"), index);
      disconnect();
      shouldReconnect = true;
      nextRetryTime = millis() + RECONNECT_DELAY;
    }
  ...
  void createNTRIPRequest(char *buffer, size_t bufferSize)
  {
    ...
    if (strlen(username) > 0 || strlen(password) > 0)
    {
      ...
      {
        AddLog(LOG_LEVEL_ERROR, PSTR("NTRIPC[%d]: Credentials too long"), index);
        buffer[0] = '\0';
        return;
      }
...
static NTRIPClient* ntripClients[NTRIP_CLIENTS] = {nullptr};

static void initializeNTRIPClients()
{
  for (uint8_t i = 0; i < NTRIP_CLIENTS; i++)
  {
    if (ntripClients[i] == nullptr)
    {
      ntripClients[i] = new NTRIPClient(i);  // Pass the index here!
    }
    ntripClients[i]->begin(NtripSettings.server_settings[i]);
  }
  AddLog(LOG_LEVEL_INFO, PSTR("NTRIP: Initialized %d NTRIP clients"), NTRIP_CLIENTS);
}

static void cleanupNTRIPClients()
{
  for (uint8_t i = 0; i < NTRIP_CLIENTS; i++)
  {
    if (ntripClients[i] != nullptr)
    {
      delete ntripClients[i];
      ntripClients[i] = nullptr;
    }
  }
}
...
static void NtripProcessNewSettings()
{
  for (uint8_t i = 0; i < NTRIP_CLIENTS; i++)
  {
    if (ntripClients[i] == nullptr && NtripSettings.server_settings[i].enabled)
    {
      ntripClients[i] = new NTRIPClient(i);
      ntripClients[i]->begin(NtripSettings.server_settings[i]);
      AddLog(LOG_LEVEL_INFO, PSTR("NTRIP[%d]: Initialized client for server %s:%d"),
             i, NtripSettings.server_settings[i].host, NtripSettings.server_settings[i].port);
    } 
    else if (ntripClients[i] != nullptr && !NtripSettings.server_settings[i].enabled)
    {
      ntripClients[i]->stop();
      delete ntripClients[i];
      ntripClients[i] = nullptr;
      AddLog(LOG_LEVEL_INFO, PSTR("NTRIP[%d]: Stopped client"), i);
    } 
    else if (ntripClients[i] != nullptr && NtripSettings.server_settings[i].enabled)
    {
      AddLog(LOG_LEVEL_INFO, PSTR("NTRIP[%d]: Reinitializing client for server %s:%d"),
             i, NtripSettings.server_settings[i].host, NtripSettings.server_settings[i].port);
      ntripClients[i]->begin(NtripSettings.server_settings[i]);
    }
  }
