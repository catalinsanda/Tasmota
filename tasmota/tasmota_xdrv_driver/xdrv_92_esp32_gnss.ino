...
#define SERIAL_INPUT_BUFFER_SIZE 16384
#define HARDWARE_SERIAL_BUFFER_SIZE 8192
...
void GNSSInit(void)
{
  ...
  gnssSerial->setRxBufferSize(HARDWARE_SERIAL_BUFFER_SIZE);
...
static void broadcastSerialData(const uint8_t *buffer, size_t length)
{
  ...
  for (auto &client : streamClients)
  {
    ...
    {
      // Prepare chunked encoding header
      char header[10];
      snprintf(header, sizeof(header), "%X\r\n", length);
      
      // Try to add header first
      size_t header_len = strlen(header);
      size_t header_added = client.client->add(header, header_len);
      
      if (header_added < header_len)
      {
        // If we can't even add the header, skip this client
        AddLog(LOG_LEVEL_DEBUG, PSTR("GNSS: Failed to add header for client, skipping"));
        continue;
      }
      
      // Try to add the data buffer
      ...
      while (remaining > 0)
      {
        size_t added = client.client->add(reinterpret_cast<const char *>(data_ptr), remaining);
        
        if (added == 0)
        {
          // No space available, try to send what we have and retry
          if (client.client->send())
          {
            // Small delay to allow buffer to clear
            delay(1);
            continue; // Try again with the remaining data
          }
          else
          {
            // Send failed, abandon this client
            AddLog(LOG_LEVEL_DEBUG, PSTR("GNSS: Send failed for client, abandoning"));
            break;
          }
        }
      ...
      // Add chunked encoding trailer
      const char *trailer = "\r\n";
      size_t trailer_added = client.client->add(trailer, 2);
      
      if (trailer_added < 2)
      {
        // Try to send what we have first, then add trailer
        if (client.client->send())
        {
          delay(1);
          client.client->add(trailer, 2);
        }
      }
      ...
      if (client.client->send())
      {
        ...
        AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("GNSS: Successfully sent %d bytes to client"), total_added);
      }
      else
      {
        AddLog(LOG_LEVEL_DEBUG, PSTR("GNSS: Final send failed for client"));
      }
    }
  }
}
...
/*********************************************************************/
static void GNSSEvery50ms(void)
{
  uint32_t current_time = Rtc.utc_time;
  static uint8_t read_buffer[SERIAL_INPUT_BUFFER_SIZE];
  
  const int MAX_READ_CYCLES = 10;
  int read_cycles = 0;

  while (gnssSerial && gnssSerial->available() > 0 && read_cycles < MAX_READ_CYCLES)
  {
    ...
    AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("GNSS: Read %d bytes from serial (cycle %d)"), bytes_read, read_cycles + 1);
    ...
    if (read_cycles < MAX_READ_CYCLES && gnssSerial->available() > 0)
    {
      yield();
    }
  }
  ...
  if (read_cycles >= MAX_READ_CYCLES && gnssSerial && gnssSerial->available() > 0)
  {
    AddLog(LOG_LEVEL_DEBUG, PSTR("GNSS: Hit max read cycles (%d) with %d bytes still available"), 
           MAX_READ_CYCLES, gnssSerial->available());
  }
}
